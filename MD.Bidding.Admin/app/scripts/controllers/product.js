...
angular
    .module('homer').controller('ProductController', ['$scope', 'ngTableParams', '$http', '$filter', '$location', 'Utils',
...
angular
    .module('homer')
    .controller('ProductBidEditController', ['$scope', 'ngTableParams', '$http', '$filter', '$location', '$log', '$interval', '$timeout',
...
angular
    .module('homer')
    .controller('ProductSearchController', ['$scope', 'ngTableParams', '$http', '$log', '$filter', 'Utils','FirstDateFieldGreaterOrEqualToSecondDateField',
...
angular
    .module('homer')
    .controller('ProductBidSearchController', ['$scope', 'ngTableParams', '$http', '$log', '$filter', 'Utils', 'FirstDateFieldGreaterOrEqualToSecondDateField',
    function ($scope, ngTableParams, $http, $log, $filter, Utils, FirstDateFieldGreaterOrEqualToSecondDateField) {
        $scope.data = [];
        $scope.fromDateGreaterThantoDate = false;
        $scope.totalAmount = 0;
        ...
        $scope.SearchProducts = function (product) {
            ...
            $scope.data = [];
            var promise = $http.post('/webapi/productApi/GenerateProductBidsReport',
                    {
                        FromDate: Utils.isUndefinedOrNull(product.FromDate) ? null : product.FromDate,
                        ToDate: Utils.isUndefinedOrNull(product.ToDate) ? null : product.ToDate,
                        SearchText: product.SearchText
            ...
            promise.then(
             function (payload) {
                 ...
                 $scope.totalAmount = 0;
                 if ($scope.data.length > 0) {                    
                         $scope.totalAmount = $scope.data.length
                         $scope.messageDisplayed = "Total Number of Bids Made for different products from " + product.FromDate + "  to " + product.ToDate + "is :"
                 }
                 $scope.tableParams.reload();
             });
        ...
        $scope.GenerateReportForProductsSoldInPastSevenDays = function () {
            $scope.data = [];
            var promise = $http.get('/webapi/productApi/GenerateReportForProductBidsSoldInPastSevenDays', {});
            ...
            promise.then(
             function (payload) {
                 ...
                 if ($scope.data.length > 0) {
                     $scope.totalAmount = $scope.data.length
                     $scope.messageDisplayed = "Total Number of Products Bids Made  In the Past Seven Days is :";
                 }
                 $scope.tableParams.reload();
             });
        }
        ...
        $scope.GenerateTodaysProductReport = function () {
            $scope.data = [];
            var promise = $http.get('/webapi/productApi/GenerateTodaysProductBidsReport', {});
            promise.then(
             function (payload) {
                 $scope.data = payload.data;
                 $scope.totalAmount = 0;
                 if ($scope.data.length > 0) {
                     $scope.totalAmount = $scope.data.length
                     $scope.messageDisplayed = "Total Number of Product Bids Made Today is :";
                 }
                 $scope.tableParams.reload();
             });
        }
        ...
        $scope.GenerateYesterdayProductReport = function () {
            $scope.data = [];
            var promise = $http.get('/webapi/productApi/GenerateYesterdayProductBidsReport', {});
            promise.then(
             function (payload) {
                 $scope.data = payload.data;
                 $scope.totalAmount = 0;
                 if ($scope.data.length > 0) {
                     $scope.totalAmount = $scope.data.length
                     $scope.messageDisplayed = "Total Product Bids Made Yesterday is :";
                 }
                 $scope.tableParams.reload();
             });
        }
        ...
        $scope.GenerateCurrentWeekReport = function () {
            $scope.data = [];
            var promise = $http.get('/webapi/productApi/GenerateCurrentWeekProductBidsReport', {});
            promise.then(
             function (payload) {
                 $scope.data = payload.data;
                 $scope.totalAmount = 0;
                 if ($scope.data.length > 0) {
                     ...
                     $scope.messageDisplayed = "Total Product Bids Made In This Week is  :";
                 }
                 $scope.tableParams.reload();
             });
        }
        ...
        $scope.GenerateCurrentMonthProductReport = function () {
            $scope.data = [];
            var promise = $http.get('/webapi/productApi/GenerateCurrentMonthProductBidsReport', {});
            promise.then(
             function (payload) {
                 $scope.data = payload.data;
                 $scope.totalAmount = 0;
                 if ($scope.data.length > 0) {
                     $scope.totalAmount = $scope.data.length
                     $scope.messageDisplayed = "Total Products Bids Made In The Current Month is :";
                 }
                 $scope.tableParams.reload();
             });
        }
